"""
Model Verification: It is important to verify any numerical solution against a known,
analytical solution to ensure the numerical model is correct. There are several options to do
this (such as the 1D homogeneous problems, or radial flow around the well). For this project
you are required to verify your numerical model with a 1D homogenous problem.
Mathematically the validation problem is given as:

$ \frac{\phi\mu c_t}{k} \frac{\partial p}{\partial t} =
\frac{\partial^2 p}{\partial x^2}  $
Initial condition:
$ p(x, 0) = p_i$
Boundary conditions:
p(x_{wf}, t) = p_{wf},
\frac{\partial p}{\partial x} @ x=x_e = 0,
